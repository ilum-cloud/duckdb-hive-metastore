...
This extension, HiveMetastore, allow you to ... <extension_goal>.
...
Now we can use the features from the extension directly in DuckDB. The template contains a single scalar function `hive_metastore()` that takes a string arguments and returns a string:
```
D select hive_metastore('Jane') as result;
┌───────────────┐
│    result     │
│    varchar    │
├───────────────┤
